...
var ASM_CONSTS = {
  4149744: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4149805: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4149869: function() {return Module.webglContextAttributes.powerPreference;},  
 4149927: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4149982: function($0) {performance.now = function() { return $0; };},  
 4150030: function($0) {performance.now = function() { return $0; };},  
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _onTelegramInitial( iOnly, iName, iMethod, iUnpkg ) {
          ...
          if ( window.integration_telegram ) {
              ...
              let oScriptElement = document.createElement( "script" );
              oStyleElement.textContent = "html { overflow: hidden; } body { width: 100%; height: var(--telegram-vh, 100%); overflow: hidden; padding: 0px; margin: 0px;} #unity-container { top: var(--telegram-top, 0px); } #tc-widget-root > tc-root > div.s-enter-to { overflow: hidden; }";
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", async () => {
                  ...
                  Reflect.set( oObject, "onScrollTo", isPhone ? async function () {
                      window.scrollTo( {
                      ...
                      //document.documentElement.style.setProperty( "--telegram-top", `${ ( document.documentElement.scrollHeight || document.body.scrollHeight ) - window.innerHeight }px` );
                      document.documentElement.style.setProperty( "--telegram-vh", `${ window.innerHeight * 2 }px` );
                      document.documentElement.style.setProperty( "--telegram-top", `0px` );
                      await new Promise( ( oResolve ) => setTimeout( oResolve, 200 ) );
                      window.scrollTo( { top: ( document.documentElement.scrollHeight || document.body.scrollHeight ) - window.innerHeight } );
                  } : async function () {
                      window.scrollTo( {
                          top: ( document.documentElement.scrollHeight || document.body.scrollHeight ) * 2,
                          behavior: "smooth"
