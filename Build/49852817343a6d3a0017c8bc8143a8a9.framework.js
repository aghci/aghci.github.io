 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  function _onTelegramInitial( iOnly, iName, iMethod, iUnpkg ) {
          ...
          if ( window.integration_telegram ) {
              ...
              let oScriptElement = document.createElement( "script" );
              oStyleElement.textContent = "html { overflow: hidden; } body { width: 100%; height: calc(var(--telegram-vh, 1vh) * 100); overflow: hidden; padding: 0px; margin: 0px;} #unity-container { top: var(--telegram-top, 0px); }";
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", async () => {
                  ...
                  do {
                      ...
                      document.documentElement.style.setProperty( "--telegram-vh", `${ window.innerHeight * 0.01 }px` );
                      ...
                      await new Promise( ( oResolve ) => requestAnimationFrame ? requestAnimationFrame( oResolve ) : setTimeout( oResolve, 10 ) );
                      alert( ( document.documentElement.scrollHeight || document.body.scrollHeight ) - window.innerHeight );
                  } while ( true );
              } );
              oScriptElement.addEventListener( "error", () => alert( `load "${ sUnpkg }" error` ) );
