...
var ASM_CONSTS = {
 ...
 4159310: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  async function _onSystemClipboard( iOnly, iValue ) {
          ...
          await new Promise( ( resolve ) => requestAnimationFrame ? requestAnimationFrame( resolve ) : setTimeout( resolve, 0 ) );
  
          let isResult = !/iPhone|iPad|iPod/i.test( navigator.userAgent ) || confirm( window.integration_system.tips );
  ...
  function _onSystemInitial( iOnly, iName, iMethod, iTips ) {
          let sOnly = UTF8ToString( iOnly );
          let sTips = UTF8ToString( iTips );
