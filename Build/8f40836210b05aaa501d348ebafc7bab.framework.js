  ...
  async function _onSystemClipboard(iOnly, iValue) {
          ...
          try {
              if (sValue == "") {
                  ...
                  await window.integration_system.clipboard(sValue);
                  oObject = {
                      Only: sOnly,
                      Type: "Success",
                      Value: ""
          ...
          if (window.integration_system) {
              ...
              oScriptElement.onload = () => {
                  ...
                  oClipboardJS.on('success', () => {
                      window.integration_system.complete.forEach(t => t.apply(window, [true]));
                      window.integration_system.complete.length = 0;
                  });
                  oClipboardJS.on('error', () => {
                      window.integration_system.complete.forEach(t => t.apply(window, [false]));
                      window.integration_system.complete.length = 0;
                  });
                  SendMessage(window.integration_system.name, window.integration_system.method, JSON.stringify({
                      Only: sOnly,
                      Type: "Success",
                      Value: ""
                  }));
              };
              let oDispatch = function () {
                  // 获取按钮的位置
              ...
              window.addEventListener("click", function () {
                  // window.integration_system.trigger.forEach(t => t.apply(window, null));
                  // window.integration_system.trigger.length = 0;
                  alert(2);
              }, false);
              document.querySelector("#unity-canvas").addEventListener(() => {
                  // window.integration_system.trigger.forEach(t => t.apply(window, null));
                  // window.integration_system.trigger.length = 0;
                  alert(1);
              }, false);
              ...
              window.integration_system.clipboard = function (sValue) {
                  alert(3);
                  window.integration_system.trigger.push(oDispatch);
                  oTextareaElement.value = sValue;
                  return new Promise((resolve, reject) => window.integration_system.complete.push((isStatus) => isStatus ? resolve() : reject()));
              };
