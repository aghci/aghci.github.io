...
var ASM_CONSTS = {
  4158432: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4158493: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4158557: function() {return Module.webglContextAttributes.powerPreference;},  
 4158615: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4158670: function($0) {performance.now = function() { return $0; };},  
 4158718: function($0) {performance.now = function() { return $0; };},  
 4158766: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _onSystemClipboard(iOnly, iValue) {
          Reflect.get(window, "#unity_bridge_system").onClipboard(iOnly, iValue);
      }

  function _onSystemInitial(iOnly, iName, iMethod) {
          if (Reflect.has(window, "#unity_bridge_system")) {
              ...
              var Bridge = (function() {
                  ...
                  Bridge.prototype.onClipboard = function(iOnly, iValue) {
                      return __awaiter(this, void 0, void 0, function() {
              ...
              Reflect.set(window, "#unity_bridge_system", new Bridge(iOnly, iName, iMethod));
          }
      }

  function _onSystemPrompt(iOnly, iTitle, iValue) {
          Reflect.get(window, "#unity_bridge_system").onPrompt(iOnly, iTitle, iValue);
      }

  function _onSystemVisibility(iOnly, iName, iMethod) {
          Reflect.get(window, "#unity_bridge_system").onVisibility(iOnly, iName, iMethod);
      }

  function _onTelegramDisableClosingConfirmation(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onDisableClosingConfirmation(iOnly);
      }

  function _onTelegramEnableClosingConfirmation(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onEnableClosingConfirmation(iOnly);
      }

  function _onTelegramExpand(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onExpand(iOnly);
      }

  function _onTelegramInitData(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onInitData(iOnly);
      }

  function _onTelegramInitial(iOnly, iName, iMethod, iUnpkg) {
          if (Reflect.has(window, "#unity_bridge_telegram")) {
              SendMessage(UTF8ToString(iName), UTF8ToString(iMethod), JSON.stringify({
                  Only: UTF8ToString(iOnly),
                  Type: "Failure",
              ...
              Reflect.set(window, "#unity_bridge_telegram", new Bridge(iOnly, iName, iMethod, iUnpkg));
          }
      }

  function _onTelegramOpenInvoice(iOnly, iUrl) {
          Reflect.get(window, "#unity_bridge_telegram").onOpenInvoice(iOnly, iUrl);
      }

  function _onTelegramOpenLink(iOnly, iUrl, isTryInstantView) {
          Reflect.get(window, "#unity_bridge_telegram").onOpenLink(iOnly, iUrl, isTryInstantView);
      }

  function _onTelegramOpenTelegramLink(iOnly, iUrl) {
          Reflect.get(window, "#unity_bridge_telegram").onOpenTelegramLink(iOnly, iUrl);
      }

  function _onTelegramStartApp(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onStartApp(iOnly);
      }

  function _onTelegramUser(iOnly) {
          Reflect.get(window, "#unity_bridge_telegram").onUser(iOnly);
      }

  function _onThinkingInitial(iOnly, iName, iMethod, iScript, iAppId, iServerUrl, isPageShow, isPageHide) {
          if (Reflect.has(window, "#unity_bridge_thinking")) {
              SendMessage(UTF8ToString(iName), UTF8ToString(iMethod), JSON.stringify({
                  Only: UTF8ToString(iOnly),
                  Type: "Failure",
              ...
              Reflect.set(window, "#unity_bridge_thinking", new Bridge(iOnly, iName, iMethod, iScript, iAppId, iServerUrl, isPageShow, isPageHide));
          }
      }

  function _onThinkingLogin(iOnly, iAccountId) {
          Reflect.get(window, "#unity_bridge_thinking").onLogin(iOnly, iAccountId);
      }

  function _onThinkingQuick(iOnly, iQuick, iParam) {
          Reflect.get(window, "#unity_bridge_thinking").onQuick(iOnly, iQuick, iParam);
      }

  function _onThinkingTrack(iOnly, iEvent, iParam) {
          Reflect.get(window, "#unity_bridge_thinking").onTrack(iOnly, iEvent, iParam);
      }

  function _onThinkingUserSet(iOnly, iParam) {
          Reflect.get(window, "#unity_bridge_thinking").onUserSet(iOnly, iParam);
      }

  function _onThinkingUserSetOnce(iOnly, iParam) {
          Reflect.get(window, "#unity_bridge_thinking").onUserSetOnce(iOnly, iParam);
      }

  function _onTonAddress(iOnly) {
          Reflect.get(window, "#unity_bridge_ton").onAddress(iOnly);
      }

  function _onTonDisconnect(iOnly) {
          Reflect.get(window, "#unity_bridge_ton").onDisconnect(iOnly);
      }

  function _onTonInitial(iOnly, iName, iMethod, iUrl, iUnpkg, iManifest) {
          if (Reflect.has(window, "#unity_bridge_ton")) {
              SendMessage(UTF8ToString(iName), UTF8ToString(iMethod), JSON.stringify({
                  Only: UTF8ToString(iOnly),
                  Type: "Failure",
              ...
              Reflect.set(window, "#unity_bridge_ton", new Bridge(iOnly, iName, iMethod, iUrl, iUnpkg, iManifest));
          }
      }

  function _onTonStatus(iOnly) {
          Reflect.get(window, "#unity_bridge_ton").onStatus(iOnly);
      }

  function _onTonTransaction(iOnly, iValidUntil, iValue) {
          Reflect.get(window, "#unity_bridge_ton").onTransaction(iOnly, iValidUntil, iValue);
      }
