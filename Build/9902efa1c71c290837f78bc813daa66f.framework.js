 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  function _onTelegramInitial( iOnly, iName, iMethod, iUnpkg ) {
          ...
          if ( window.integration_telegram ) {
              ...
              let oScriptElement = document.createElement( "script" );
              //oStyleElement.textContent = "html { overflow: hidden; } body { width: 100%; height: 100vh; overflow: hidden auto; padding: 0px; margin: 0px;} #unity-container { top: 0px; }";
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", () => {
                  let oObject = Object.create( window.Telegram.WebApp );
                  Reflect.set( oObject, "reply", oReply );
                  Reflect.set( oObject, "onTelegramViewportChanged", function ( oValue ) {
                      console.log( oValue );
                  } );
                  ...
                  Reflect.set( oObject, "onTelegramExpand", function ( sOnly ) {
                      let oObject;
                      try {
                          alert( `viewportStableHeight: ${ this.viewportStableHeight }\r\ninnerHeight         : ${ window.innerHeight }` );
                          this.expand();
                          window.scrollTo( {
                              top: document.documentElement.scrollHeight || document.body.scrollHeight,
                              behavior: 'smooth'
                          } );
                          setTimeout( () => alert( `viewportStableHeight: ${ this.viewportStableHeight }\r\ninnerHeight         : ${ window.innerHeight }` ), 5000 );
                          oObject = {
                              Only: sOnly,
                              Type: "Success",
                              Value: ""
                  ...
                  oObject.ready();
                  oObject.onEvent( "viewportChanged", oObject.onTelegramViewportChanged );
                  oObject.reply( {
                      Only: sOnly,
                      Type: "Success",
                      Value: ""
