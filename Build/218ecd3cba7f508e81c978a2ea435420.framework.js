...
var ASM_CONSTS = {
 ...
 4159406: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  async function _onSystemClipboard(iOnly, iValue) {
          ...
          let isResult = !/iPhone|iPad|iPod/i.test(navigator.userAgent) || await window.integration_system.onClipboard();
          ...
          if (window.integration_system) {
              ...
              window.integration_system = {
                  ...
                  clipboard: async (sValue) => {
                      ...
                      try {
                          let oCallback = {};
                          oClipboardJS.on('success', (oValue) => {
                              Reflect.get(oCallback, "resolve").call(oCallback, oValue.text);
                              oValue.clearSelection();
                          });
                  ...
                  onClipboard: async () => {
                      let oDivElement = document.createElement("div");
                      oDivElement.style.position = "absolute";
                      oDivElement.style.zIndex = "9999";
                      oDivElement.style.top = "0px";
                      oDivElement.style.right = "0px";
                      oDivElement.style.bottom = "0px";
                      oDivElement.style.left = "0px";
                      let oBodyElement = document.querySelector("body");
                      oBodyElement.appendChild(oDivElement);
                      let isStatus;
                      try {
                          await new Promise(resolve => {
                              oDivElement.addEventListener("click", resolve);
                              setTimeout(() => oDivElement.click(), 200);
                          });
                          isStatus = true;
                      } catch {
                          isStatus = false;
                      }
                      oBodyElement.removeChild(oDivElement);
                      return isStatus;
                  }
              };
