...
var ASM_CONSTS = {
 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  ( iOnly ) => window.integration_telegram.onTelegramDisableClosingConfirmation( UTF8ToString( iOnly ) )

  ( iOnly ) => window.integration_telegram.onTelegramEnableClosingConfirmation( UTF8ToString( iOnly ) )

  ( iOnly ) => window.integration_telegram.onTelegramExpand( UTF8ToString( iOnly ) )

  ( iOnly ) => window.integration_telegram.onTelegramInitData( UTF8ToString( iOnly ) )
  ...
  ( iOnly, iUrl ) => window.integration_telegram.onTelegramOpenInvoice( UTF8ToString( iOnly ), UTF8ToString( iUrl ) )

  ( iOnly, iUrl, isTryInstantView ) => window.integration_telegram.onTelegramOpenLink( UTF8ToString( iOnly ), UTF8ToString( iUrl ), isTryInstantView )

  ( iOnly, iUrl ) => window.integration_telegram.onTelegramOpenTelegramLink( UTF8ToString( iOnly ), UTF8ToString( iUrl ) )

  ( iOnly ) => window.integration_telegram.onTelegramStartApp( UTF8ToString( iOnly ) )

  ( iOnly ) => window.integration_telegram.onTelegramUser( UTF8ToString( iOnly ) )
