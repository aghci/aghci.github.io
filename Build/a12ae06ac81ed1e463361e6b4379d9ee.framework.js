...
var ASM_CONSTS = {
 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  function _onTelegramDisableClosingConfirmation( iOnly ) {
          ...
          window.integration_telegram.reply( oObject );
      }
  ...
  function _onTelegramExpand( iOnly ) {
          ...
          let oObject;
          try {
              alert( `viewportStableHeight: ${ window.Telegram.WebApp.viewportStableHeight }\r\ninnerHeight         : ${ window.innerHeight }` );
              window.Telegram.WebApp.expand();
              window.scrollTo( {
                  top: document.documentElement.scrollHeight || document.body.scrollHeight,
                  behavior: 'smooth'
              } );
              setTimeout( () => alert( `viewportStableHeight: ${ window.Telegram.WebApp.viewportStableHeight }\r\ninnerHeight         : ${ window.innerHeight }` ), 5000 );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
                  Value: ""
  ...
  function _onTelegramOpenInvoice( iOnly, iUrl ) {
          ...
          try {
              window.Telegram.WebApp.openInvoice( sUrl, sStatus => window.integration_telegram.reply( {
                  Only: sOnly,
                  Type: "Success",
                  Value: sStatus
              } ) );
          } catch ( error ) {
              window.integration_telegram.reply( {
                  Only: sOnly,
                  Type: "Failure",
                  Value: typeof error == "string" ? error : error instanceof Error ? error.message : JSON.stringify( error )
              } );
