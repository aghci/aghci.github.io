...
var ASM_CONSTS = {
  4159072: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4159133: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4159197: function() {return Module.webglContextAttributes.powerPreference;},  
 4159255: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4159310: function($0) {performance.now = function() { return $0; };},  
 4159358: function($0) {performance.now = function() { return $0; };},  
 4159406: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  async function _onSystemClipboard( iOnly, iValue ) {
          let sOnly = UTF8ToString( iOnly );
          let sValue = UTF8ToString( iValue );
  
          //let isResult = !/iPhone|iPad|iPod/i.test( navigator.userAgent ) || confirm( `"${ sValue }"\r\nConfirm copying this text?` );
          let isResult = !/iPhone|iPad|iPod/i.test( navigator.userAgent ) || await window.integration_system.confirm();
  
          if ( isResult && await window.integration_system.clipboard( sValue ) ) {
              oObject = {
                  Only: sOnly,
                  Type: "Success",
                  Value: ""
          ...
          window.integration_system.reply( oObject );
      }

  function _onSystemInitial( iOnly, iName, iMethod ) {
          let sOnly = UTF8ToString( iOnly );
          let sUnpkg = "https://cdnjs.cloudflare.com/ajax/libs/clipboard.js/2.0.11/clipboard.min.js";
          let oReply = ( ( sName, sMethod, oObject ) => SendMessage( sName, sMethod, JSON.stringify( oObject ) ) ).bind( window, UTF8ToString( iName ), UTF8ToString( iMethod ) );
          if ( window.integration_system ) {
              oReply.apply( window, {
                  Only: sOnly,
                  Type: "Failure",
              ...
              window.integration_system = {
                  reply: oReply,
                  clipboard: async ( sValue ) => {
                      let oButtonElement = document.createElement( "button" );
                      oButtonElement.id = "clipboard";
                      oButtonElement.setAttribute( "data-clipboard-text", sValue );
                      oButtonElement.textContent = "copy";
                      let oBodyElement = document.querySelector( "body" );
                      oBodyElement.appendChild( oButtonElement );
                      let oClipboardJS = new ClipboardJS( "#clipboard" );
                      let isStatus;
                      try {
                          let oCallback = {};
                          oClipboardJS.on( 'success', ( oValue ) => Reflect.get( oCallback, "resolve" ).call( oCallback, oValue.text ) );
                          oClipboardJS.on( 'error', () => Reflect.get( oCallback, "reject" ).call( oCallback ) );
                          await new Promise( ( resolve, reject ) => {
                              Reflect.set( oCallback, "resolve", resolve );
                              Reflect.set( oCallback, "reject", reject );
                  ...
                  confirm: async () => {
                      let oButtonElement = document.createElement( "div" );
                      oButtonElement.style.position = "absolute";
                      oButtonElement.style.zIndex = "9999";
                      oButtonElement.style.top = "0px";
                      oButtonElement.style.right = "0px";
                      oButtonElement.style.bottom = "0px";
                      oButtonElement.style.left = "0px";
                      let oBodyElement = document.querySelector( "body" );
                      oBodyElement.appendChild( oButtonElement );
                      await new new Promise( ( resolve ) => oButtonElement.addEventListener( "click", resolve ) );
                      oBodyElement.removeChild( oButtonElement );
                  }
              };
              let oHeadElement = document.querySelector( "head" );
              let oScriptElement = document.createElement( "script" );
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", () => window.integration_system.reply( {
                  Only: sOnly,
                  Type: "Success",
                  Value: ""
              } ) );
              oScriptElement.addEventListener( "error", () => alert( `load "${ sUnpkg }" error` ) );
  ...
  function _onSystemPrompt( iOnly, iTitle, iValue ) {
          let sOnly = UTF8ToString( iOnly );
          let sTitle = UTF8ToString( iTitle );
          let sValue = UTF8ToString( iValue );
          try {
              let sContent = prompt( sTitle, sValue );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
                  Value: sContent
  ...
  function _onSystemVisibility( iOnly, iName, iMethod ) {
          let sOnly = UTF8ToString( iOnly );
          let sName = UTF8ToString( iName );
          let sMethod = UTF8ToString( iMethod );
          let oObject1;
          try {
              document.addEventListener( "visibilitychange", () => {
                  ...
                  SendMessage( sName, sMethod, oObject2 );
              } );
              oObject1 = {
                  Only: sOnly,
                  Type: "Success",
          ...
          window.integration_system.reply( oObject1 );
      }

  function _onTelegramDisableClosingConfirmation( iOnly ) {
          window.integration_telegram.onTelegramDisableClosingConfirmation( UTF8ToString( iOnly ) );
      }

  function _onTelegramEnableClosingConfirmation( iOnly ) {
          window.integration_telegram.onTelegramEnableClosingConfirmation( UTF8ToString( iOnly ) );
      }

  function _onTelegramExpand( iOnly ) {
          window.integration_telegram.onTelegramExpand( UTF8ToString( iOnly ) );
      }

  function _onTelegramInitData( iOnly ) {
          window.integration_telegram.onTelegramInitData( UTF8ToString( iOnly ) );
      }

  function _onTelegramInitial( iOnly, iName, iMethod, iUnpkg ) {
          let sOnly = UTF8ToString( iOnly );
          let sUnpkg = UTF8ToString( iUnpkg );
          let oReply = ( ( sName, sMethod, oObject ) => SendMessage( sName, sMethod, JSON.stringify( oObject ) ) ).bind( window, UTF8ToString( iName ), UTF8ToString( iMethod ) );
          if ( window.integration_telegram ) {
              oReply.apply( window, {
                  Only: sOnly,
                  Type: "Failure",
              ...
              let oHeadElement = document.querySelector( "head" );
              let oStyleElement = document.createElement( "style" );
              let oScriptElement = document.createElement( "script" );
              ...
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", async () => {
                  let oObject = Object.create( window.Telegram.WebApp );
                  Reflect.set( oObject, "reply", oReply );
                  Reflect.set( oObject, "onTelegramInitData", function ( sOnly ) {
                      let oObject;
                      try {
                          oObject = {
                              Only: sOnly,
                  ...
                  Reflect.set( oObject, "onTelegramStartApp", function ( sOnly ) {
                      let oObject;
                      try {
                          oObject = {
                              Only: sOnly,
                              Type: "Success",
                              Value: this.initDataUnsafe.start_param
                          }
                      } catch ( error ) {
                          oObject = {
                              Only: sOnly,
                  ...
                  Reflect.set( oObject, "onTelegramUser", function ( sOnly ) {
                      let oObject;
                      try {
                          if ( this.initDataUnsafe.user ) {
                              oObject = {
                                  Only: sOnly,
                                  Type: "Success",
                                  Value: JSON.stringify( this.initDataUnsafe.user )
                              }
                          } else {
                              oObject = {
                                  Only: sOnly,
                  ...
                  Reflect.set( oObject, "onTelegramReceiver", function ( sOnly ) {
                      let oObject;
                      try {
                          if ( this.initDataUnsafe.receiver ) {
                              oObject = {
                                  Only: sOnly,
                                  Type: "Success",
                                  Value: JSON.stringify( this.initDataUnsafe.receiver )
                              }
                          } else {
                              oObject = {
                                  Only: sOnly,
                  ...
                  Reflect.set( oObject, "onTelegramExpand", function ( sOnly ) {
                      let oObject;
                      try {
                          this.expand();
                          this.onScrollTo();
                  ...
                  Reflect.set( oObject, "onTelegramEnableClosingConfirmation", function ( sOnly ) {
                      let oObject;
                      try {
                  ...
                  Reflect.set( oObject, "onTelegramDisableClosingConfirmation", function ( sOnly ) {
                      let oObject;
                      try {
                          this.onDisableClosingConfirmation();
                          oObject = {
                              Only: sOnly,
                              Type: "Success",
                              Value: ""
                  ...
                  Reflect.set( oObject, "onTelegramOpenLink", function ( sOnly, sUrl, isTryInstantView ) {
                      let oObject;
                      try {
                          this.openLink( sUrl, { try_instant_view: isTryInstantView } );
                          oObject = {
                              Only: sOnly,
                              Type: "Success",
                              Value: ""
                  ...
                  Reflect.set( oObject, "onTelegramOpenTelegramLink", function ( sOnly, sUrl ) {
                      let oObject;
                      try {
                          this.openTelegramLink( sUrl );
                          oObject = {
                              Only: sOnly,
                              Type: "Success",
                              Value: ""
                  ...
                  Reflect.set( oObject, "onTelegramOpenInvoice", function ( sOnly, sUrl ) {
                      try {
                          this.openInvoice( sUrl, sStatus => this.reply( {
                              Only: sOnly,
                              Type: "Success",
                              Value: sStatus
                          } ) );
                  ...
                  let isPhone = /iPhone|iPad|iPod/i.test( navigator.userAgent );
                  let iTop = 0;
                  Reflect.set( oObject, "onScrollTo", isPhone ? function () {
                      window.scrollTo( {
                      ...
                      document.documentElement.style.setProperty( "--telegram-vh", `${ window.innerHeight }px` );
                      document.documentElement.style.setProperty( "--telegram-top", `${ iTop }px` );
                  } : function () {
                      window.scrollTo( {
                          top: ( document.documentElement.scrollHeight || document.body.scrollHeight ) * 2,
                          behavior: "smooth"
                      } );
                      document.documentElement.style.setProperty( "--telegram-vh", `${ window.innerHeight * 2 }px` );
                      document.documentElement.style.setProperty( "--telegram-top", `${ iTop }px` );
                  } );
                  Reflect.set( window, "integration_telegram", oObject );
                  oObject.ready();
                  oObject.reply( {
                      Only: sOnly,
                      Type: "Success",
                  ...
                  do {
                      oObject.onScrollTo();
                      await new Promise( ( oResolve ) => setTimeout( oResolve, 1000 ) );
                  } while ( true );
              } );
              oScriptElement.addEventListener( "error", () => alert( `load "${ sUnpkg }" error` ) );
  ...
  function _onTelegramOpenInvoice( iOnly, iUrl ) {
          window.integration_telegram.onTelegramOpenInvoice( UTF8ToString( iOnly ), UTF8ToString( iUrl ) );
      }

  function _onTelegramOpenLink( iOnly, iUrl, isTryInstantView ) {
          window.integration_telegram.onTelegramOpenLink( UTF8ToString( iOnly ), UTF8ToString( iUrl ), isTryInstantView );
      }

  function _onTelegramOpenTelegramLink( iOnly, iUrl ) {
          window.integration_telegram.onTelegramOpenTelegramLink( UTF8ToString( iOnly ), UTF8ToString( iUrl ) );
      }

  function _onTelegramStartApp( iOnly ) {
          window.integration_telegram.onTelegramStartApp( UTF8ToString( iOnly ) );
      }

  function _onTelegramUser( iOnly ) {
          window.integration_telegram.onTelegramUser( UTF8ToString( iOnly ) );
      }

  function _onThinkingInitial( iOnly, iName, iMethod, iScript, iAppId, iServerUrl, isPageShow, isPageHide ) {
          let sOnly = UTF8ToString( iOnly );
          let sScript = UTF8ToString( iScript );
          let sAppId = UTF8ToString( iAppId );
          let sServerUrl = UTF8ToString( iServerUrl );
          let oReply = ( ( sName, sMethod, oObject ) => SendMessage( sName, sMethod, JSON.stringify( oObject ) ) ).bind( window, UTF8ToString( iName ), UTF8ToString( iMethod ) );
          if ( window.integration_thinking ) {
              oReply.apply( window, {
                  Only: sOnly,
                  Type: "Failure",
              ...
              oScriptElement.addEventListener( "load", () => {
                  window.integration_thinking.ta = thinkingdata;
                  window.integration_thinking.ta.init( {
                      appId: sAppId,
                  ...
                  window.integration_thinking.reply( {
                      Only: sOnly,
                      Type: "Success",
                      Value: ""
                  } );
              }, false );
              oScriptElement.addEventListener( "error", () => alert( `load "${ sScript }" error` ) );
              oHeadElement.appendChild( oScriptElement );
  ...
  function _onThinkingLogin( iOnly, iAccountId ) {
          let sOnly = UTF8ToString( iOnly );
          let sAccountId = UTF8ToString( iAccountId );
          let oObject;
          if ( window.integration_thinking.ta ) {
              window.integration_thinking.ta.login( sAccountId );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
          ...
          window.integration_thinking.reply( oObject );
      }

  function _onThinkingQuick( iOnly, iQuick, iParam ) {
          let sOnly = UTF8ToString( iOnly );
          let sQuick = UTF8ToString( iQuick );
          let sParam = UTF8ToString( iParam );
          let oObject;
          if ( window.integration_thinking.ta ) {
              if ( sParam == "" ) {
                  window.integration_thinking.ta.quick( sQuick );
              }
              else {
                  window.integration_thinking.ta.quick( sQuick, JSON.parse( sParam ) );
              }
              oObject = {
                  Only: sOnly,
                  Type: "Success",
  ...
  function _onThinkingTrack( iOnly, iEvent, iParam ) {
          let sOnly = UTF8ToString( iOnly );
          let sEvent = UTF8ToString( iEvent );
          let sParam = UTF8ToString( iParam );
          let oObject;
          if ( window.integration_thinking.ta ) {
              if ( sParam == "" ) {
                  window.integration_thinking.ta.track( sEvent );
              }
              else {
                  window.integration_thinking.ta.track( sEvent, JSON.parse( sParam ) );
              }
              oObject = {
                  Only: sOnly,
                  Type: "Success",
  ...
  function _onThinkingUserSet( iOnly, iParam ) {
          ...
          let oObject;
          if ( window.integration_thinking.ta ) {
              window.integration_thinking.ta.userSet( JSON.parse( sParam ) );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
  ...
  function _onThinkingUserSetOnce( iOnly, iParam ) {
          ...
          let oObject;
          if ( window.integration_thinking.ta ) {
              window.integration_thinking.ta.userSetOnce( JSON.parse( sParam ) );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
  ...
  async function _onTonAddress( iOnly ) {
          let sOnly = UTF8ToString( iOnly );
          if ( window.integration_ton.ui ) {
              if ( window.integration_ton.ui.connected && window.integration_ton.wallet ) {
                  window.integration_ton.reply( {
                      Only: sOnly,
                      Type: "Success",
                      Value: window.TON_CONNECT_UI.toUserFriendlyAddress( window.integration_ton.wallet.account.address )
                  } );
              } else {
                  let oDivElement = document.getElementById( "tc-widget-root" );
                  oDivElement.style.display = "block";
                  await window.integration_ton.ui.openModal();
                  window.integration_ton.address = sOnly;
              }
          } else {
              window.integration_ton.reply( {
                  Only: sOnly,
  ...
  async function _onTonDisconnect( iOnly ) {
          let sOnly = UTF8ToString( iOnly );
          let oObject;
          if ( window.integration_ton.ui ) {
              if ( window.integration_ton.ui.connected ) {
                  try {
                      await window.integration_ton.ui.disconnect();
                      oObject = {
                          Only: sOnly,
                          Type: "Success",
                          Value: `${ !window.integration_ton.ui.connected }`
                      };
                  } catch ( error ) {
                      oObject = {
                          Only: sOnly,
          ...
          window.integration_ton.reply( oObject );
      }

  function _onTonInitial( iOnly, iName, iMethod, iUrl, iUnpkg, iManifest ) {
          let sOnly = UTF8ToString( iOnly );
          let sUrl = UTF8ToString( iUrl );
          let sUnpkg = UTF8ToString( iUnpkg );
          let sManifest = UTF8ToString( iManifest );
          let oReply = ( ( sName, sMethod, oObject ) => SendMessage( sName, sMethod, JSON.stringify( oObject ) ) ).bind( window, UTF8ToString( iName ), UTF8ToString( iMethod ) );
          if ( window.integration_ton ) {
              oReply.apply( window, {
                  Only: sOnly,
                  Type: "Failure",
              ...
              window.integration_ton = {
                  url: sUrl,
                  reply: oReply,
                  onStatusChange: oWallet => window.integration_ton.wallet = oWallet,
                  onModalStateChange: oState => {
                      if ( oState.status == "closed" ) {
                          let oObject;
                          if ( window.integration_ton.wallet ) {
                              oObject = {
                                  Only: window.integration_ton.address,
                                  Type: "Success",
                                  Value: window.TON_CONNECT_UI.toUserFriendlyAddress( window.integration_ton.wallet.account.address )
                              };
                          } else {
                          ...
                          window.integration_ton.address = null;
                          window.integration_ton.wallet = null;
                      }
                  }
              };
              let oHeadElement = document.querySelector( "head" );
              ...
              oScriptElement.addEventListener( "load", async () => {
                  ...
                  let oRegex3 = /^http/;
                  let oRegex4 = /^(.+)\/*$/;
                  window.integration_ton.ui = new TON_CONNECT_UI.TonConnectUI( {
                      manifestUrl: oRegex1.test( sManifest ) ? `${ location.protocol }${ sManifest }` : oRegex2.test( sManifest ) ? `${ location.protocol }//${ location.host }${ sManifest }` : oRegex3.test( sManifest ) ? sManifest : `${ location.protocol }//${ location.host }${ location.pathname.repeat( oRegex4, "$1" ) }/${ sManifest }`,
                      actionsConfiguration: { twaReturnUrl: `${ window.integration_ton.url }?startapp=address` }
                  } );
                  window.integration_ton.ui.onStatusChange( window.integration_ton.onStatusChange );
                  window.integration_ton.ui.onModalStateChange( window.integration_ton.onModalStateChange );
                  await window.integration_ton.ui.connectionRestored;
                  window.integration_ton.reply( {
                      Only: sOnly,
                      Type: "Success",
                      Value: ""
  ...
  async function _onTonTransaction( iOnly, iValidUntil, iValue ) {
          let sOnly = UTF8ToString( iOnly );
          let sValue = UTF8ToString( iValue );
          let oObject;
          try {
              let oResult = await window.integration_ton.ui.sendTransaction( {
                  validUntil: Math.floor( Date.now() / 1000 ) + iValidUntil,
                  messages: JSON.parse( atob( sValue ) )
              }, { twaReturnUrl: `${ window.integration_ton.url }?startapp=transaction` } );
              oObject = {
                  Only: sOnly,
                  Type: "Success",
                  Value: oResult.boc
