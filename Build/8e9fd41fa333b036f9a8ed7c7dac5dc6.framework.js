 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  function _onTelegramInitial( iOnly, iName, iMethod, iUnpkg ) {
          ...
          if ( window.integration_telegram ) {
              ...
              let oScriptElement = document.createElement( "script" );
              oStyleElement.textContent = "html { overflow: hidden; } body { width: 100%; height: var(--telegram-vh, 100%); overflow: hidden; padding: 0px; margin: 0px;} #unity-container { top: var(--telegram-top, 0px); }";
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener( "load", async () => {
                  ...
                  Reflect.set( oObject, "onScrollTo", isPhone ? function () {
                      window.scrollTo( { top: ( document.documentElement.scrollHeight || document.body.scrollHeight ) * 2 } );
                  ...
                  let iTimeout = isPhone ? 1000 : 100;
                  Reflect.set( window, "integration_telegram", oObject );
                  do {
                      oObject.onScrollTo();
                      await new Promise( ( oResolve ) => setTimeout( oResolve, iTimeout ) );
                  } while ( true );
              } );
              oScriptElement.addEventListener( "error", () => alert( `load "${ sUnpkg }" error` ) );
