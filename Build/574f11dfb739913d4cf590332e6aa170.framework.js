 ...
 4150078: function() {performance.now = Module['emscripten_get_now_backup'];}
  ...
  function _onTelegramInitial(iOnly, iName, iMethod, iUnpkg) {
          ...
          if (window.integration_telegram) {
              ...
              let oScriptElement = document.createElement("script");
              oStyleElement.textContent = "html { overflow: hidden; } body { width: 100%; height: calc(var(--telegram-vh, 1vh) * 100); overflow: hidden; padding: 0px; margin: 0px;} #unity-container { top: 0px; }";
              oScriptElement.src = sUnpkg;
              oScriptElement.addEventListener("load", () => {
                  ...
                  oObject.ready();
                  oObject.onEvent("viewportChanged", (oValue) => {
                      window.scrollTo({ top: document.documentElement.scrollHeight || document.body.scrollHeight });
                      let vh = window.innerHeight * 0.01;
                      document.documentElement.style.setProperty('--telegram-vh', `${vh}px`);
                      if (oObject.isExpanded && oValue.isStateStable) {
